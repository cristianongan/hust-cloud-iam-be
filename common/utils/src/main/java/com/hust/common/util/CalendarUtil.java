...
public class CalendarUtil {
	...
	/** The month ids. */
    private static final int[] MONTH_IDS = new int[] {Calendar.JANUARY, Calendar.FEBRUARY, Calendar.MARCH, Calendar.APRIL,
			Calendar.MAY, Calendar.JUNE, Calendar.JULY, Calendar.AUGUST, Calendar.SEPTEMBER, Calendar.OCTOBER,
			Calendar.NOVEMBER, Calendar.DECEMBER};

	/** The calendar pool. */
	private static final Map<String, String[]> calendarPool = new ConcurrentHashMap<>();
	...
	public static String[] getDays(Locale locale, String pattern) {
		...
		String[] days = calendarPool.get(key);
	...
	public static int getGregorianDay(Calendar cal) {
		...
		return (int) (6286 + year * 365.25 - (double)year / 100 + (double)year / 400 + 30.6 * month + 0.2 + day);
	}
	...
	/**
	 * Gets the GT Instant of another day.
	 * 
	 * @param cal
	 * @return
	 */
	...
	/**
	 * Gets the GT Instant of month.
	 * 
	 * @param cal
	 * @return
	 */
	public static Instant getGTInstantOfMonth(Calendar cal) {
		Calendar gtCal = (Calendar) cal.clone();
	...
	/**
	 * Gets the LT Instant of month.
	 * 
	 * @param cal
	 * @return
	 */
	public static Instant getLTInstantOfMonth(Calendar cal) {
		Calendar ltCal = (Calendar) cal.clone();

		ltCal.set(Calendar.DAY_OF_MONTH, ltCal.getActualMaximum(Calendar.DAY_OF_MONTH));

		return getLTInstant(ltCal);
	}
	...
	public static String[] getMonths(Locale locale, String pattern) {
		...
		String[] months = calendarPool.get(key);
	...
	public static boolean isBroadcastDate(int month, int day, int year) {
		...
		return !(cal2.get(Calendar.DAY_OF_WEEK) != Calendar.MONDAY
	...
	private static long getTimeInMillis(Date date) {
		...
		return cal.getTimeInMillis() / TimeUtil.DAY;
