package org.mbg.common.security;
...
public class RsaProvider implements Serializable {
	...
	public RsaProvider(int keySize) {
		try {
			KeyPairGenerator keyGen = KeyPairGenerator.getInstance(SecurityConstants.Algorithm.RSA);
	...
	public static PublicKey getPublicKey(String base64PublicKey) {
		try {
			...
			return KeyFactory.getInstance(SecurityConstants.Algorithm.RSA).generatePublic(keySpec);
		} catch (Exception e) {
			_log.error(e.getMessage());
		}
		return null;
	...
	public static PrivateKey getPrivateKey(String base64PrivateKey) {
		try {
			...
			return KeyFactory.getInstance(SecurityConstants.Algorithm.RSA).generatePrivate(keySpec);
		} catch (Exception e) {
			_log.error(e.getMessage());
		}
	...
	public static RsaProvider fromPrivateKey(String algorithm, String textPrivateKey)
        ...
        KeyFactory keyFactory = KeyFactory.getInstance(SecurityConstants.Algorithm.RSA);
	...
	public static RsaProvider fromPublishKey(String algorithm, String textPublicKey)
	...
	private static X509EncodedKeySpec getX509EncodedKeySpec(String textKey) {
